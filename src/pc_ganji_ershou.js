...
Rent.prototype.wgetList = function(t){
    ...
    if(t.class == '3') {
        ...
        var opt = new helper.basic_options(t.cityPinyin+".ganji.com","/fang5/"+pinyin+"/o"+t.pn+"/");
        console.log("[GET ] %s, %s, %s, %d",t.cityName,t.districtName,name,t.pn);
    } else if(t.class == '2') {
        var pinyin = t.districtPinyin;
        var name = t.districtName;
        var opt = new helper.basic_options(t.cityPinyin+".ganji.com","/fang5/"+pinyin+"/");
        console.log("[GET ] %s, %s",t.cityName,t.districtName);
    } else {
        var opt = new helper.basic_options(t.cityPinyin+".ganji.com","/fang5/");
    ...
    if(!data){
	   console.log("data empty.");
	    ...
	    $("ul.list-style1 li").each(function(){
			...
			var member = $("div.list-mod3 p.list-part span.ico-bang-new",this).text()||0;
			if(member)
			    memberCount++;
			
			var record = [args[0].cityName,args[0].category,args[0].class,member,hot,jing,top,adTop,name,houseName||"",pubDate||"",url,"\n"].join();
