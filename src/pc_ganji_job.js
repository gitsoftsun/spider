...
Rent.prototype.processList = function(data,args,res){
    if(!data) {
        ...
        $("div dl.list-noimg.job-list").each(function(){
            ...
            var user = $("dd.company a",this).attr("title").trim().replace(/[\n\r,，]/g,";");
            ...
            if(member)
                memberCount++;
            var record = [t.cityName,t.cat_name,t.class,member,hot,top,adTop,pub_date,title,user,url_title,url_user,"\n"].join();
        ...
        if(t.class == '1') {
            ...
            if ($("div dl.list-noimg.job-list").length<10 || memberCount<=4) {
                console.log("[DONE] less info,Category: " + t.cat_name);
                setTimeout(function () {
                    that.wgetList();
                }, (Math.random() * 4 + 2) * 1000);
