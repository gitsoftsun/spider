...
Worker.prototype.wget = function(){
	...
	helper.request_data(cate.url,null,function(data,args,res){
...
Worker.prototype.processList = function(data,args,res){
    ...
    args[0].products = [];
    args[0].done=[""];
    $("ul.plistul li.pList-item ").each(function(){
	var id = $(this).attr('search_product_id');
	var link = $("div.num_warp_list_name a",this);
	var title = link.text().trim();
	var m = title.match(/[\d一二三四五六七八九十两]+(\.\d)?(mg|g|ml|l|kg|个|块|张|片|包|袋|瓶|盒|对|只|支|抽|枚|根|回|件套|件|套|克|毫升|升|cc|粒)(\*\d)?/gi);
	var unit = m && m.join();
	title = title && title.replace(/\t/g,'');
	var url = link.attr('href');
	var price = $("div.num_warp_list_name_mall span",this).first().text().trim();
	var discount = $("div.num_warp_list_name_mall em",this).text().trim();
	discount = discount && discount.replace(/[\(\)]/g,'') || '无';
	var prod = [that.today,url,id,title,price,discount,args[0].brand,args[0].c,unit];
	
	if(0 < $(".sale_sign.tm",this).length){// just a product. not a deal.
	    prod.push("Y");
	    prod.push('N');
	    args[0].products.push(prod);
	}else{
	    if(0 < $(".sale_sign.qg",this).length){
		prod.push("Y");
		prod.push('Y');
	    }else{
		prod.push("N");
